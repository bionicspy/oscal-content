...
- Keywords: default curated set, override with --keywords (repeatable flag and/or comma-separated phrases).
  Stored as metadata.prop(name='keywords', ns='http://csrc.nist.gov/ns/oscal', value='comma, separated, terms').

References:
- Keywords in OSCAL are expressed as a metadata property named 'keywords' (OSCAL namespace),
  with a comma-separated value.  # https://pages.nist.gov/OSCAL-Reference/models/develop/complete/json-reference/
...
# Sensible defaults if --keywords is omitted
DEFAULT_KEYWORDS = [
    "MITRE ATLAS",
    "ATLAS",
    "AI security",
...
def build_keywords_value(keywords_args, default_list) -> str:
    """
    Normalize keywords from CLI:
      - Accepts repeated --keywords flags or comma-separated values.
      - De-duplicates while preserving order.
      - Returns a single comma-separated string per OSCAL guidance.
    """
    if not keywords_args:
        parts = list(default_list)
    else:
        parts = []
        for item in keywords_args:
...
def revisions_from_atlas_releases(limit: int = 5) -> list[dict]:
    ...
    for rel in releases[:int(limit)]:
        ...
        rev = {
            ...
            "version": str(version),
            "published": rel.get("published_at"),  # ISO 8601 with 'Z' is valid in OSCAL
            "links": [
...
def build_oscal_catalog(parsed: dict, source_kind: str, source_bytes: bytes,
                        ...
                        releases_limit: int | None = 5,
                        keywords_value: str | None = None) -> dict:
    ...
    # --- Keywords (OSCAL metadata prop: name='keywords' in OSCAL namespace) ---
    # The value is a comma-separated list of keywords.  (See OSCAL reference)
    if keywords_value:
        md["props"].append({
            "name": "keywords",
...
def main():
    ...
    ap.add_argument(
        "--keywords",
        action="append",
        help="Add keywords (repeat the flag or pass comma-separated). "
             "If omitted, a curated ATLAS/OSCAL default set is used."
    )
    ...
    # Build keywords string for metadata.prop('keywords')
    kw_value = build_keywords_value(args.keywords, DEFAULT_KEYWORDS)
